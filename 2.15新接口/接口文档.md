...
```
query func getCertain_RegionLevel_SumSoc(regionLevel: RegionLevel): async [SumRESERVES] 
```
```
...
指定碳库, 返回所有数据
```
query func getCertain_Source_Data(dataSource: DataSource): async [RESERVES]
```
```
...
指定碳库,指定地区级别的所有基础碳数据
```
query func getCertanin_RegionLevel_Data(dataSource: DataSource, regionLevel: RegionLevel): async [RESERVES]
```
```
    public type DataSource = {
        #Vegetation; // 植被碳库
...
```
query func getCertain_RegionLevel_Time_Data(dataSource: DataSource, regionLevel: RegionLevel, time: Nat): async [RESERVES] 
```
```
...
指定碳库,指定地区,指定时间 基础碳数据    
```
query func getCertain_Id_Time_Data(dataSource: DataSource, id: Nat, time: Nat): async ?RESERVES 
```
```
dataSource: DataSource
